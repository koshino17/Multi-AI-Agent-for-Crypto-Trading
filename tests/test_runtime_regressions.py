...
class RuntimeRegressionTests(unittest.TestCase):
    ...
    def test_strategy_research_cycle_does_not_promote_custom_cost_candidate(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            library_path = Path(tmpdir) / "external_benchmark_library.json"
            library_path.write_text(
                json.dumps(
                    {
                        "baseline_strategy_id": "grid_range_reversion_v1",
                        "strategies": [
                            {
                                "id": "grid_range_reversion_v1",
                                "name": "Grid",
                                "kind": "rule_strategy",
                                "generator": "grid_range_reversion",
                                "source": "live",
            ...
            with tempfile.TemporaryDirectory() as storage_dir:
                storage = build_storage_layout(storage_dir)
                settings = SimpleNamespace(
                    external_benchmark_library_path=str(library_path),
                    timeframe="15m",
                    external_benchmark_slippage_pct=0.0002,
                    taker_fee_pct=0.001,
                    external_benchmark_max_alpha_signals=10,
                )
                runs = [
                    {
                        "symbol": "SOL/USDT",
