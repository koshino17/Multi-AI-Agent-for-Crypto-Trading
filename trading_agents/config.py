...
class Settings:
    ...
    external_mentor_enabled: bool = _bool("EXTERNAL_MENTOR_ENABLED", False)
    external_mentor_providers: tuple[str, ...] = _list("EXTERNAL_MENTOR_PROVIDERS", "openai,gemini")
    external_mentor_openai_model: str = os.getenv("EXTERNAL_MENTOR_OPENAI_MODEL", "gpt-5.5")
    external_mentor_gemini_model: str = os.getenv("EXTERNAL_MENTOR_GEMINI_MODEL", "gemini-2.5-flash")
    openai_api_key: str = os.getenv("OPENAI_API_KEY", "")
    mentor_timeout_seconds: float = _float("MENTOR_TIMEOUT_SECONDS", 30.0)
    mentor_autopromote_enabled: bool = _bool("MENTOR_AUTOPROMOTE_ENABLED", True)
    mentor_gate_windows: tuple[str, ...] = _list("MENTOR_GATE_WINDOWS", "96,320,1000")
    mentor_gate_min_trades: int = _int("MENTOR_GATE_MIN_TRADES", 8)
    mentor_gate_min_expectancy_delta_96: float = _float("MENTOR_GATE_MIN_EXPECTANCY_DELTA_96", 0.03)
    mentor_gate_min_expectancy_delta_long: float = _float("MENTOR_GATE_MIN_EXPECTANCY_DELTA_LONG", 0.00)
    mentor_gate_min_pf_delta_96: float = _float("MENTOR_GATE_MIN_PF_DELTA_96", 0.00)
    mentor_gate_min_pf_delta_long: float = _float("MENTOR_GATE_MIN_PF_DELTA_LONG", -0.05)
    mentor_gate_max_cum_return_gap_pct: float = _float("MENTOR_GATE_MAX_CUM_RETURN_GAP_PCT", 0.50)
    ...
    strategy_research_enabled: bool = _bool("STRATEGY_RESEARCH_ENABLED", True)
    strategy_research_refresh_hours: float = _float("STRATEGY_RESEARCH_REFRESH_HOURS", 2.0)
    strategy_research_include_alpha: bool = _bool("STRATEGY_RESEARCH_INCLUDE_ALPHA", True)
    ...
    llm_wake_min_score: float = _float("LLM_WAKE_MIN_SCORE", 4.0)
    llm_wake_position_min_score: float = _float("LLM_WAKE_POSITION_MIN_SCORE", 1.0)
    llm_wake_volatility_pct: float = _float("LLM_WAKE_VOLATILITY_PCT", 0.30)
    llm_wake_momentum_pct: float = _float("LLM_WAKE_MOMENTUM_PCT", 0.25)
    llm_wake_volume_ratio: float = _float("LLM_WAKE_VOLUME_RATIO", 1.35)
    ...
    llm_wake_trade_delta_ratio: float = _float("LLM_WAKE_TRADE_DELTA_RATIO", 0.35)
    llm_wake_large_trade_count: int = _int("LLM_WAKE_LARGE_TRADE_COUNT", 4)
    llm_wake_quiet_volatility_pct: float = _float("LLM_WAKE_QUIET_VOLATILITY_PCT", 0.15)
    llm_wake_quiet_volume_ratio: float = _float("LLM_WAKE_QUIET_VOLUME_RATIO", 1.05)
    ...
    market_structure_guard_enabled: bool = _bool("MARKET_STRUCTURE_GUARD_ENABLED", True)
    market_structure_guard_value_area_breach_bps: float = _float("MARKET_STRUCTURE_GUARD_VALUE_AREA_BREACH_BPS", 8.0)
    market_structure_guard_fvg_near_bps: float = _float("MARKET_STRUCTURE_GUARD_FVG_NEAR_BPS", 30.0)
    market_structure_guard_trade_delta_ratio: float = _float("MARKET_STRUCTURE_GUARD_TRADE_DELTA_RATIO", 0.20)
    market_structure_guard_volume_ratio: float = _float("MARKET_STRUCTURE_GUARD_VOLUME_RATIO", 1.10)
    market_structure_guard_fill_ratio: float = _float("MARKET_STRUCTURE_GUARD_FILL_RATIO", 0.25)
    po3_deterministic_score_enabled: bool = _bool("PO3_DETERMINISTIC_SCORE_ENABLED", True)
    po3_deterministic_score_delta: float = _float("PO3_DETERMINISTIC_SCORE_DELTA", 0.06)
    po3_deterministic_score_min_flow: float = _float("PO3_DETERMINISTIC_SCORE_MIN_FLOW", 0.20)
    ...
    perp_take_profit_pct: float = _float("PERP_TAKE_PROFIT_PCT", 1.8)
    ...
    perp_profit_lock_trigger_pct: float = _float("PERP_PROFIT_LOCK_TRIGGER_PCT", 0.8)
    perp_profit_lock_breakeven_offset_pct: float = _float("PERP_PROFIT_LOCK_BREAKEVEN_OFFSET_PCT", 0.25)
    perp_profit_lock_trigger_2_pct: float = _float("PERP_PROFIT_LOCK_TRIGGER_2_PCT", 1.5)
    perp_profit_lock_stop_2_pct: float = _float("PERP_PROFIT_LOCK_STOP_2_PCT", 0.60)
    ...
    intraday_report_handoff_exit_enabled: bool = _bool("INTRADAY_REPORT_HANDOFF_EXIT_ENABLED", True)
    intraday_report_handoff_exit_minutes: float = _float("INTRADAY_REPORT_HANDOFF_EXIT_MINUTES", 30.0)
    intraday_report_handoff_max_pnl_pct: float = _float("INTRADAY_REPORT_HANDOFF_MAX_PNL_PCT", 0.45)
